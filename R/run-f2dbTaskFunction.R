...
#' \item{info}{Informational messages generated by the task function.}
#' \item{warning}{Warning messages generated by the task funciton.}
#' \item{error}{Error messages generated by the task function.}
#'
...
methods::setMethod(
  ...
  function(object, input = NA, item = NA) {
    ...
    debug("taskCall: ", rlang::quo_name(object@taskCall))
    ...
    logWarning <- function(cnd) {
      warnings <<- append(warnings, rlang::cnd_message(cnd))
      debug(name(object), ": ", rlang::cnd_message(cnd))
      rlang::cnd_muffle(cnd)
    }

    logInfo <- function(cnd) {
      infos <<- append(infos, rlang::cnd_message(cnd))
      debug(name(object), ": ", rlang::cnd_message(cnd))
      rlang::cnd_muffle(cnd)
    }
    ...
    tryCatch(
      error = function(cnd) {
        success <<- FALSE
        errors <<- append(errors, rlang::cnd_message(cnd))
        debug(name(object), ": ", rlang::cnd_message(cnd))
      },
      withCallingHandlers(
        message = logInfo,
    ...
    list(success = success, output = output, info = infos, warning = warnings, error = errors)
  }
)
